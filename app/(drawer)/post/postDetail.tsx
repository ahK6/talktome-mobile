...
import { getcommentsByPostId, postDetail } from "@/store/posts/posts.actions";
import { IComment, IPostDetail, PostTypes } from "@/store/posts/posts.types";
...
import { setActiveTab, setSelectedKeyword } from "@/store/posts/posts.store";
...
const PostDetail = () => {
    ...
    const checkAuthAndLoadData = async () => {
        ...
        if (isVisible) {
            await Promise.all([getPostDetail(postId), getPostComments(postId)]);
        }
    };

    const getPostDetail = async (postId: string) => {
        if (!postDetailInfo || currentPostId !== postId) {
            setLoading(true);
        }
        try {
            const res = await dispatch(
                postDetail({ inputParams: { idPost: postId } })
            ).unwrap();
    ...
    const handleCategoryPress = (keyword: string) => {
        const postType = postDetailInfo?.type as PostTypes;
        const tabIndex = postType === "helping" ? 1 : 0;

        dispatch(setSelectedKeyword({ keyword, type: postType }));

        dispatch(setActiveTab(tabIndex));

        if (postType === "helping") {
            router.replace("/(drawer)/(tabs)/testimonies");
        } else {
